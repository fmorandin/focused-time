...
Identifiers **excluded** from the 4-char minimum: `id`, `URL` (these may stay short).
...
```text
Focused Timer/Focused Timer/
├── FocusedTimerApp.swift       # App entry point + AppDelegate
├── ContentView.swift           # Root SwiftUI view
├── Timer/                      # Timer feature (Model, View, ViewModel)
├── Settings/                   # Settings feature (Model, View, ViewModel)
├── Help/                       # Help/onboarding views
└── Shared/
    ├── Constants/              # Localizable.xcstrings, UserDefaultKeys, etc.
    ├── Extensions/             # SwiftUI/Foundation extensions
    ├── Helpers/                # LocalNotificationManager
    ├── Network/                # NetworkManager (UserDefaults wrapper)
...
- **Unit tests** (`Focused TimerTests/`) use **Swift Testing** — `@Suite`, `@Test`, `#expect`, `#require`. Not XCTest.
- **UI tests** (`Focused TimerUITests/`) use XCTest with the `BaseFeature` base class.

## Logging

Use `Logger(subsystem: Bundle.main.bundleIdentifier!, category: String(describing: Self.self))` from `import os`. Never use `print` for logging.

## Deployment Target

iOS 26.0 (Xcode 26 SDK). Minimum iOS 26 APIs are available.
